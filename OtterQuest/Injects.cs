...
namespace OtterQuest
{
    internal class Injects
    {
        ...
        public static void NoCellCost(bool enable)
        {
            ...
            IntPtr patchAddr = WindowsInfo.rqProcess.MainModule.BaseAddress + 0x13E99FA;
            byte[] buffer = new byte[6];
            int readBytes = 0;

            if(!enable)
            {
                restore = [ 0x29, 0x91, 0xd0, 0x88, 0x00, 0x00 ];
                WriteProcessMemory(WindowsInfo.rqHandle, patchAddr, restore, restore.Length, ref readBytes);
                return;
            }
